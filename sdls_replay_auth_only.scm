(herald "SDLS: Authentication Only Replay Properties"
	(limit 100))

(defprotocol sdls-replay-auth basic
	...
	(defrole spacecraft
		...
		(trace
			...
			;; Send a transmission with the SN, payload, and MAC consisting of the SN and payload
			;; Abstraction of Security Header -> SN
			;; Abstraction of Security Trailer -> MAC only from SN and Payload
			;; Key management is out of scope for protocol, but it is a long-term agreed-upon key
			(send (cat sc gs seq payload (hash (ltk sc gs) seq payload)))
	...
	(defrole groundstation
		...
		(trace
			...
			(recv (cat sc gs seq payload (hash (ltk sc gs) seq payload)))
...
(defskeleton sdls-replay-auth
	(vars (sc gs name))
	(defstrand groundstation 3 (sc sc) (gs gs) (seq "0"))
	(defstrand groundstation 3 (sc sc) (gs gs) (seq "0"))
	(proceeds ((0 2) (1 0)))
	(non-orig (ltk sc gs)))
