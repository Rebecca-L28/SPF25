(herald "BSP: PIB Only Replay Properties"
	(limit 100))
...
(defprotocol bsp-pib basic
	...
	(defrole spacecraft
		...
		(trace
			...
			(send (cat sc gs time payload (enc (hash sc gs time payload) (invk (pubk sc))))))
		(gen-st timeLoc (cat sc gs time)))
	...
	(defrole groundstation
		...
		(trace
			...
			(recv (cat sc gs time payload (enc (hash sc gs time payload) (invk (pubk sc)))))
			;; Load current time
			(load timeLoc (cat sc gs time)))
		(gen-st timeLoc (cat sc gs time)))
