(herald "SDLS: AEAD Replay Properties"
	(limit 30))

(defprotocol sdls-replay-aead basic
	...
	(defrole spacecraft
		...
		(trace
			...
			(send (cat sc gs seq (enc payload (ltk sc gs)) (hash (ltk sc gs) seq payload)))
	...
	(defrole groundstation
		...
		(trace
			...
			(recv (cat sc gs seq (enc payload (ltk sc gs)) (hash (ltk sc gs) seq payload)))
