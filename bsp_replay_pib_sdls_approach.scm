(herald "BSP: PIB Replay Properties"
	(limit 100))

(defprotocol bsp-pib basic
	...
	(defrole spacecraft
		...
		(trace
			...
			;; Send a transmission with the SN, payload, and signature consisting of the SN and payload
			;; Key management is out of scope for protocol, but it is a long-term agreed-upon key
			(send (cat sc gs seq payload (enc (hash sc gs "+1" seq payload) (invk (pubk sc)))))
	...
	(defrole groundstation
		...
		(trace
			...
			(recv (cat sc gs seq payload (enc (hash sc gs "+1" seq payload) (invk (pubk sc)))))
